<div class="tabla-filtro" id="tabla-filtro">
        ...
        <form class="filtro-formulario" id="filtro-formulario" action="{{ route($route . '_filter') }}"
            ...
            @foreach ($filtros as $key => $values)
                ...
                @if ($key == 'category')
                    <div class="filtro-formulario-item">
                        <div class="filtro-formulario-item-campo">
                            <select name="category_id" placeholder="--categoría--" class="filtro-formulario-item-campo"
                                id="selector-categoria">
                                <option value="all" class="campo-option" id="all">Todas</option>
                                @foreach ($values as $value)
                                    <option value="{{ $value->id }}" class="campo-option" id="{{ $value->id }}">
                                        {{ $value->nombre }}
                                    </option>
                                @endforeach
                ...
                @if ($key == 'search')
                    <div class="filtro-formulario-item">
                        <div class="filtro-formulario-item-campo">
                            <input type="text" name="search" value="" id="campo-input-text" class="campo-input-text"
                                placeholder="Buscar...">
                        </div>
                    </div>
                @endif
    ...
    {{-- <div class="tabla-filtro-botones">
        <div class="tabla-filtro-boton abrir-filtro active" id="abrir-filtro">
        ...
        <div class="tabla-filtro-boton aplicar-filtro" id="aplicar-filtro">
            <svg viewBox="0 0 24 24">
                <path
...
{{-- <div class="filtros-container">
    ...
    <div class="filtros-container-second-group">
                ...
                <select 
                    ...
                    class="filter-select form-control primary-select-related">
                        ...
                        @foreach ($subfilter as $subfilter_input)
                            <option value="{{$subfilter_input->id}}" name='{{$subfilter_input->nombre}}'>{{$subfilter_input->nombre}}</option>
                        @endforeach
                </select>
            </div>
        @endisset     
