        ...
        @foreach ($tags as $tag_element)
            <div class="tabla-contenido-fila contents swipe-element " >
                <div class="tabla-contenido-fila-campos contents swipe-front promote-layer grid-column-1">
                    {{-- <div class="tabla-celda grid-column-1 ">{{ $tag_element->id }}</div> --}}
                    <div class="tabla-celda grid-column-2 ">{{ $tag_element->group }}</div>
                    <div class="tabla-celda grid-column-3 ">{{ $tag_element->key }}</div>
                    {{-- <div class="tabla-celda grid-column-4 ">{{ Carbon\Carbon::parse($tag_element->created_at)->format('d-m-Y') }}</div> --}}
                </div>
                ...
                <div class="tabla-contenido-fila-iconos contents">
                    {{-- <div class="tabla-celda swipe-back swipe-delete boton-eliminar grid-column-5" id="swipe-delete"
                        data-url="{{ route('tags_destroy', ['tag' => $tag_element->id]) }}">
                    ...
                    <div class="tabla-celda swipe-back swipe-edit boton-editar grid-column-6" id="swipe-edit"
                        data-url="{{route('tags_show')}}"
                        data-group="{{ $tag_element->group }}"
                        data-key="{{ $tag_element->key }}">
                        <svg viewBox="0 0 24 24">
                            <path fill="currentColor"
